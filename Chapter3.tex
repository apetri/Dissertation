...
\label{chp:3}
In this Chapter we describe the relevant numerical methods for performing simulations of WL observations. We use publicly available software to trace the non--linear time evolution of the matter density contrast $\delta(t)$. The geodesic equation can then be solved numerically adding the multiple deflections which light experiences traveling from sources to observers. The solution of the geodesic equation for a sufficient number of light rays allows to reconstruct the spatial profiles of the WL observables $\kappa,\pmb{\gamma},\omega$. We conclude the Chapter by presenting our ray--tracing software \LT, which we have publicly released for the use of the WL community.
...
\end{equation}
%
This initial condition (which includes baryon effects) is then evolved according to Dark Matter only collision--free dynamics. Random realizations of $\tilde{\delta}$ are drawn from $\mathcal{N}(0,P_\delta(\bb{k},z_{\rm in}))$ and the peculiar particle velocities $\bb{v}$ are assigned according to 
