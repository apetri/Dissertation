...
\end{equation} 
%
The notation in equation (\ref{eq:1:sigma8}) means that $P_\delta^{\rm lin}$ is calculated from $\delta(k,z=0)$ obtained with the linear evolution described by equation equation (\ref{eq:1:delta-lin}). $\sigma_8$ is defined from the variance of the linearly evolved density contrast smoothed with a Gaussian window of size $r_8=8\,{\rm Mpc}/h$. 
%
\begin{table}
\begin{center}
